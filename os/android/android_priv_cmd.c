...
#define AND_WARN(fmt, ...)     LOG_W("Android[%s:%d][%d]"fmt, __func__,__LINE__, pnic_info->ndev_id, ##__VA_ARGS__)
#define AND_ERR(fmt, ...)      LOG_E("Android[%s:%d][%d]"fmt, __func__,__LINE__, pnic_info->ndev_id, ##__VA_ARGS__)
...
static int android_cmd_null(nic_info_st *pnic_info, char *command,
                            int total_len, android_cmd_handle_st *cmd_handle)
{
    AND_INFO("Android private command [%s] is null\n", command);
    return 0;
}

static int android_cmd_getrssi(nic_info_st *pnic_info, char *command,
                               int total_len, android_cmd_handle_st *cmd_handle)
{
    zt_u8 qual, level;
    int bytes_written = 0;
    ...
    if (is_connected)
    {
        ...
        bytes_written +=
            zt_snprintf(&command[bytes_written], total_len, "%s rssi %d",
                        pcur_network->ssid.data, translate_percentage_to_dbm(level));
    }
    else
    {
        bytes_written += zt_snprintf(&command[bytes_written], total_len, "No connection");
    }

    AND_DBG("Process of android private command [%s] end\n", command);

    return bytes_written;
}

static int android_cmd_getlinkspeed(nic_info_st *pnic_info, char *command,
                                    int total_len, android_cmd_handle_st *cmd_handle)
{
    u16 link_speed = 0;
    int bytes_written = 0;
    ...
    if (is_connected)
    {
        zt_wlan_get_max_rate(pnic_info, (zt_u8 *)pcur_network->bssid, &link_speed);
        bytes_written += zt_snprintf(command, total_len, "LinkSpeed %d", link_speed / 10);
    }
    else
    {
        bytes_written += zt_snprintf(command, total_len, "LinkSpeed 0");
    }
...
static int android_cmd_getp2pdevaddr(nic_info_st *pnic_info, char *command,
                                     int total_len, android_cmd_handle_st *cmd_handle)
{
    struct net_device *pndev = pnic_info->ndev;

    zt_memcpy(command, pndev->dev_addr, ETH_ALEN);
    AND_DBG("Process of android private command [%s] end\n", command);
    return ETH_ALEN;
}


static int android_cmd_setfwpath(nic_info_st *pnic_info, char *command,
                                 int total_len, android_cmd_handle_st *cmd_handle)
{
...
static int android_cmd_setband(nic_info_st *pnic_info, char *command,
                               int total_len, android_cmd_handle_st *cmd_handle)
{
    ...
    if (sscanf(value, "%u", &band) >= 1)
    {
        ...
        AND_INFO("Our driver only support 2.4g:%u\n", band);
        return 0;
    }
...
static int android_cmd_getband(nic_info_st *pnic_info, char *command,
                               int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;

    bytes_written = zt_snprintf(command, total_len, "%u", pnic_info->setband);

    return bytes_written;
}

/* set country code*/
extern int country_code_to_ch_plan(char *pcountry_code, zt_u8 *pch_plan);
...
static int andriod_set_countrycode(nic_info_st *pnic_info, char *country_code)
{
    ...
    if (ret < 0)
    {
        AND_WARN("country_code_to_ch_plan failed ret =  :%d\n", ret);
        return ret;
    }
    else
    {
        ...
        AND_DBG("set channel_plan = %x : ret = %d\n", ch_plan, ret);
    }
    return ret;
    
}

static int android_cmd_setcountry(nic_info_st *pnic_info, char *command,
                                  int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    ...
    {
        AND_WARN("set country code failed !!!\n");
    }
...
/* need realise later*/
static int android_cmd_set_mgnt_wpsp2pie(nic_info_st *pnic_info, char *command,
        int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    ...
    bytes_written = zt_p2p_parse_ie(pnic_info, command + skip, total_len - skip,
                                    *(command + skip - 2) - '0');
#endif

    AND_DBG(" command [%s]  done\n", command);
...
/* need realise later*/
static int android_cmd_set_miracastmode(nic_info_st *pnic_info, char *command,
                                        int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef CONFIG_WFD
                         ...
                         int max, int *bytes_left)
{
    ...
    while (*bytes_left > 0)
    {
        ...
        *bytes_left -= 1;
        ...
        {
            ssid[idx].SSID_len = 0;
            zt_memset((char *)ssid[idx].SSID, 0x0, WLAN_SSID_MAXLEN);
            *bytes_left -= 1;
            str += 1;

            AND_INFO("BROADCAST SCAN  left=%d\n", *bytes_left);
        }
        else if (str[0] <= WLAN_SSID_MAXLEN)
        {
            ...
            *bytes_left -= 1;
            ...
            {
                AND_INFO("%s out of memory range len=%d but left=%d\n",
                         __func__, ssid[idx].SSID_len, *bytes_left);
                return -1;
            }

            zt_memcpy((char *)ssid[idx].SSID, str, ssid[idx].SSID_len);

            *bytes_left -= ssid[idx].SSID_len;
            str += ssid[idx].SSID_len;

            AND_INFO("%s :size=%d left=%d\n",
                     (char *)ssid[idx].SSID, ssid[idx].SSID_len, *bytes_left);
        }
        else
        {
            AND_INFO("### SSID size more that %d\n", str[0]);
            return -1;
        }

        if (idx++ > max)
        {
            AND_INFO("%s number of SSIDs more that %d\n", __func__, idx);
            return -1;
        }
    }
...
static int do_aly_cipher_list(struct pno_nlo_info *nlo_info, char *list_str)
{
    ...
    while (strlen(pch) != 0)
    {
        ...
        if (pnext != NULL)
        {
            ...
            pend = strstr(pch, "}");
            if (zt_strncmp(pch, CIPHER_NONE, strlen(CIPHER_NONE)) == 0)
            {
                nlo_info->ssid_cipher_info[index] = 0x00;
            }
            else if (zt_strncmp(pch, CIPHER_WPA_PSK,
                                strlen(CIPHER_WPA_PSK)) == 0)
            {
                nlo_info->ssid_cipher_info[index] = 0x66;
            }
            else if (zt_strncmp(pch, CIPHER_WPA_EAP,
                       ...
                       int pno_repeat, int pno_freq_expo_max)
{
    ...
    nlo_info->ssid_num = num & ZT_U32_BIT_LEN_MASK(8);
    nlo_info->hidden_ssid_num = num & ZT_U32_BIT_LEN_MASK(8);
    ...
    if (IS_ERR(fp))
    {
        AND_WARN("Error, wpa_supplicant.conf doesn't exist.\n");
        AND_WARN("Error, cipher array using default value.\n");
        return 0;
    }
    ...
    if (len < 0 || len > 2048)
    {
        AND_WARN("Error, file size is bigger than 2048.\n");
        AND_WARN("Error, cipher array using default value.\n");
        return 0;
    }
                             ...
                             unsigned short bw_mode)
{
    ...
    AND_DBG(" channel_num: %d, orig_ch: %d, orig_bw: %d orig_40_offset: %d\n",
                ...
                int pno_time, int pno_repeat, int pno_freq_expo_max, u8 tag)
{
    ...
    */
    ...
    if (tag)
    {
        if (num == 0)
        {
            AND_INFO("%s, nssid is zero, no need to setup pno ssid list\n",
                     __func__);
            return 0;
        }
        ...
        if (pwrctl == NULL)
        {
            AND_INFO("%s, ERROR: pwrctl is NULL\n", __func__);
            return -1;
        }
        else
        {
        ...
        {
            AND_INFO("%s, ERROR: alloc nlo_info, ssid_list, scan_info fail\n",
                     __func__);
            goto failing;
        }
...
void dbg_dev_pno(struct net_device *net, u8 tag)
{
    ...
    if (tag)
    {
        ...
        AND_INFO("fast_scan_iterations: %d\n",
                 pwrctl->pnlo_info->fast_scan_iterations);
        AND_INFO("fast_scan_period: %d\n", pwrctl->pnlo_info->fast_scan_period);
        AND_INFO("slow_scan_period: %d\n", pwrctl->pnlo_info->slow_scan_period);
        AND_INFO("ssid_length: ");
        ...
        AND_INFO("cipher_info: ");
        for (i = 0; i < MAX_PNO_LIST_COUNT; i++)
        {
            AND_INFO("%d, ", pwrctl->pnlo_info->ssid_cipher_info[i]);
        }
        AND_INFO("\n");

        AND_INFO("channel_info: ");
        for (i = 0; i < MAX_PNO_LIST_COUNT; i++)
        {
            AND_INFO("%d, ", pwrctl->pnlo_info->ssid_channel_info[i]);
        }
        AND_INFO("\n");
        ...
        AND_INFO("orig 40 offset: %d\n", pwrctl->pscan_info->orig_40_offset);
        for (i = 0; i < MAX_SCAN_LIST_COUNT; i++)
        {
            AND_INFO("[%02d] avtive:%d, timeout:%d, tx_power:%d, ch:%02d\n",
...
#ifdef CONFIG_PNO_SUPPORT
static int android_cmd_pnosetup(nic_info_st *pnic_info, char *command,
                                int total_len, android_cmd_handle_st *cmd_handle)

{
    ...
    {
        AND_INFO("%s argument=%d less min size\n", __func__, total_len);
        goto exit_proc;
    }
    ...
    zt_memset(pno_ssids_local, 0, sizeof(pno_ssids_local));
            ...
            (cmd_tlv_temp->subver == PNO_TLV_SUBVERSION))
    {
        ...
        if ((nssid = do_aly_ssid_list_tlv(&str_ptr, pno_ssids_local,
                                          ...
                                          &tlv_size_left)) <= 0)
        {
            AND_INFO("SSID is not presented or corrupted ret=%d\n", nssid);
            goto exit_proc;
        }
        else
        {
            ...
            {
                AND_INFO("%s scan duration corrupted field size %d\n",
                         __func__, tlv_size_left);
                goto exit_proc;
            }
            str_ptr++;
            pno_time = simple_strtoul(str_ptr, &str_ptr, 16);
            AND_INFO("%s: pno_time=%d\n", __func__, pno_time);

            if (str_ptr[0] != 0)
            {
                if ((str_ptr[0] != PNO_TLV_FREQ_REPEAT))
                {
                    AND_INFO("%s pno repeat : corrupted field\n", __func__);
                    goto exit_proc;
                }
                str_ptr++;
                pno_repeat = simple_strtoul(str_ptr, &str_ptr, 16);
                AND_INFO("%s :got pno_repeat=%d\n", __FUNCTION__, pno_repeat);
                if (str_ptr[0] != PNO_TLV_FREQ_EXPO_MAX)
                {
                    AND_INFO("%s FREQ_EXPO_MAX corrupted field size\n",
                             __func__);
                    goto exit_proc;
                }
                str_ptr++;
                pno_freq_expo_max = simple_strtoul(str_ptr, &str_ptr, 16);
                AND_INFO("%s: pno_freq_expo_max=%d\n",
    ...
    {
        AND_INFO("%s get wrong TLV command\n", __FUNCTION__);
        goto exit_proc;
    }
...
static int android_cmd_getmacaddr(nic_info_st *pnic_info, char *command,
                                  int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    struct net_device *pndev = pnic_info->ndev;

    bytes_written = zt_snprintf(command, total_len, "Macaddr = " MAC_FMT,
                                MAC_ARG(pndev->dev_addr));
    AND_DBG("Process of android private command [%s] end\n", command);
    return bytes_written;
}

static int android_cmd_setblockscan(nic_info_st *pnic_info, char *command,
                                    int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef CONFIG_IOCTL_CFG80211
    ...
    pwdev_info->block_scan = (*block_value == '0') ? zt_false : zt_true;
#endif
...
static int android_cmd_setblock(nic_info_st *pnic_info, char *command,
                                int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef CONFIG_IOCTL_CFG80211
    ...
    pwdev_info->block = (*block_value == '0') ? zt_false : zt_true;
#endif
...
static int android_cmd_wfd_enable(nic_info_st *pnic_info, char *command,
                                  int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef CONFIG_WFD
    ...
    if (pwfd_info->wfd_enable == zt_false)
    {
        zt_p2p_wfd_enable(pnic_info, zt_true);
    }
#endif
...
static int android_cmd_wfd_disable(nic_info_st *pnic_info, char *command,
                                   int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef CONFIG_WFD
    ...
    {
        zt_p2p_wfd_enable(pnic_info, zt_false);
    }
#endif
...
int parse_command(char *pcmd)
{
    ...
    return zt_atoi(pcmd + i) ;
}


static int android_cmd_set_tcpport(nic_info_st *pnic_info, char *command,
                                   int total_len, android_cmd_handle_st *cmd_handle)
{
#ifdef CONFIG_WFD
    zt_p2p_wfd_set_ctrl_port(pnic_info, parse_command(command));
#endif

    return 0;
}

static int android_cmd_set_wfd_devtype(nic_info_st *pnic_info, char *command,
                                       int total_len, android_cmd_handle_st *cmd_handle)
{
#ifdef CONFIG_WFD
    p2p_info_st *p2p_info = pnic_info->p2p;
    ...
    pwfd_info->wfd_device_type = (zt_u8)parse_command(command);
...
/*need realise*/
static int android_cmd_changedtim(nic_info_st *pnic_info, char *command,
                                  int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    pwr_info_st *pwr_info = pnic_info->pwr_info;
    zt_u8 dtim = zt_atoi(command + 9); //valid (0-16]
    ...
    zt_mcu_set_lps_opt(pnic_info, dtim); // not support current, dtim is fixed to 4.
#else
    lps_set_fw_power_mode(pnic_info, PWR_MODE_ACTIVE);
#endif
    AND_DBG("Process of android private command [%s] end, dtim:%d\n", command,
            dtim);
#endif
    return bytes_written;
}
...
static int android_cmd_set_macacl(nic_info_st *pnic_info, char *command,
                                  int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    //zt_wlan_info_t *pwlan_info = pnic_info->wlan_info;
    //zt_wlan_network_t *pcur_network = &pwlan_info->cur_network;
    AND_DBG("Process of android private command [%s] end\n", command);
    return bytes_written;
}

static int android_cmd_p2p_disable(nic_info_st *pnic_info, char *command,
                                   int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
#ifdef ZT_CONFIG_P2P
    p2p_info_st *p2p_info = pnic_info->p2p;
    if (p2p_info->p2p_enabled == zt_true)
    {
        zt_p2p_reset(pnic_info);
        zt_p2p_disable(pnic_info);
    }

    AND_DBG("Process of android private command [%s] end,p2p_enable=%d\n", command,
            p2p_info->p2p_enabled);
#endif
    return bytes_written;
}


static int android_cmd_driverversion(nic_info_st *pnic_info, char *command,
                                     int total_len, android_cmd_handle_st *cmd_handle)
{
    int bytes_written = 0;
    bytes_written = zt_snprintf(command, strlen(DRIVER_VERSION) + 1,
                                DRIVER_VERSION);
...
int zt_android_priv_cmd_ioctl(struct net_device *net, struct ifreq *ifr,
                              int cmd)
{
    int ret         = 0;
    int cmd_num;
    ...
    AND_DBG("used_len:%d,total_len:%d", priv_cmd.used_len, priv_cmd.total_len);
    ...
    command = zt_kzalloc(priv_cmd.total_len);
    if (!command)
    {
    ...
    for (cmd_num = 0; cmd_num < ANDROID_CMD_SIZE; cmd_num++)
    {
        ...
        {
            bytes_written = gl_android_cmd[cmd_num].func(pnic_info, command,
                            priv_cmd.total_len, &gl_android_cmd[cmd_num]);
        }
        else if (cmd_num == ANDROID_CMD_SIZE)
        {
            AND_WARN("Unknown PRIVATE command %s - ignored\n", command);
            zt_snprintf(command, 3, "OK");
    ...
    if (bytes_written >= 0)
    {
        ...
        AND_DBG("copy date to user: %s\n", command);
        if (copy_to_user((void *)priv_cmd.buf, command, bytes_written))
        {
            AND_WARN("failed to copy data to user buffer\n");
            ret = -EFAULT;
        }
    }
    else
